...
import { fetchComplaints, updateComplaint as apiUpdateComplaint } from '@/lib/api';
...
const AdminDashboard = () => {
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const updated = await apiUpdateComplaint(id, { status });
      setComplaints(prev => prev.map(item => item.id === id ? updated : item));
      toast({
        title: status === 'resolved' ? "Issue Resolved!" : "Status Updated!",
        description: status === 'resolved' ? "Citizen has been awarded 10 points." : undefined,
      });
      setSelectedComplaint(null);
    } catch (error) {
      toast({ title: error.message || "Unable to update status", variant: 'destructive' });
    }
  };
  
  const handleAssignDepartment = async (id, department) => {
    try {
      const updated = await apiUpdateComplaint(id, { assignedDepartment: department, status: 'assigned' });
      setComplaints(prev => prev.map(item => item.id === id ? updated : item));
      toast({ title: "Department Assigned!" });
      setSelectedComplaint(null);
    } catch (error) {
      toast({ title: error.message || "Unable to assign department", variant: 'destructive' });
    }
  };
