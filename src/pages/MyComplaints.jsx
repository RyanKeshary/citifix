...
const MyComplaints = () => {
    ...
    useEffect(() => {
        ...
        const loadComplaints = async () => {
            try {
                const userComplaints = await fetchMyComplaints();
                if (active) {
    ...
    const getStatusChip = (status) => {
        switch(status) {
            ...
            case 'escalated': return <span className="bg-purple-700 text-white px-2 py-1 rounded-full text-xs font-bold flex items-center gap-1"><CheckCircle className="w-3 h-3"/> Escalated</span>
            default: return null;
        }
    }
