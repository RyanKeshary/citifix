...
import { fetchComplaints, voteComplaint as apiVoteComplaint } from '@/lib/api';
    ...
    useEffect(() => {
        ...
        const loadComplaints = async () => {
            try {
                ...
                if (active) {
                    setComplaints(sortComplaints(allComplaints, filter));
                }
            } catch (error) {
                if (active) {
                    setComplaints([]);
    ...
    const handleVote = (id) => {
        ...
        apiVoteComplaint(id)
            .then((updated) => {
                ...
                setComplaints((current) => sortComplaints(current.map((complaint) => complaint.id === updated.id ? updated : complaint), filter));
            })
            .catch((error) => {
                toast({ title: error.message || 'Could not submit vote', variant: 'destructive' });
            });
    };
